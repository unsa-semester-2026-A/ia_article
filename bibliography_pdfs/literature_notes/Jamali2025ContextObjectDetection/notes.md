...
### 1. Introducción
La detección de objetos en visión artificial se enfrenta a retos persistentes como la similitud interclase, variaciones intraclase, condiciones de iluminación adversas, objetos fuera de su entorno típico (out-of-context) y oclusiones. El uso de contexto ayuda a reducir el espacio de búsqueda espacial y semántica, imitando el sistema visual de los humanos.
...
### 2. Context
Define el término "contexto" en el ámbito de la visión computacional como cualquier información visual o no visual que complementa las características locales de un objeto para refinar su clasificación e inferencia espacial.
...
* **Soluciones alcanzadas**: Modelar el contexto para guiar la atención espacial hacia regiones probables del objeto y resolver oclusiones mediante información circundante.
...
* **Problemas atacados**: Predicciones fuera de contexto que degradan el rendimiento de detección.
...
Compara relaciones espaciales entre pares de objetos directos (pairwise) y relaciones complejas grupales (higher-order).
* **Problemas atacados**: Modelado de escenas con alta densidad de interacciones dinámicas.
* **Limitaciones de ese entonces**: Limitación de representar interacciones solo como pares independientes, ignorando la estructura contextual grupal.
...
#### 2.4 Context types
Categorización detallada de los tipos de contexto según su naturaleza física o informacional: Semántico, Espacial, Temporal, De Escala, Espectral y otros.
...
* **Limitaciones de ese entonces**: Dependencia exclusiva del detector en características visuales intrínsecas que colapsa ante oclusiones.
...
* **Soluciones alcanzadas**: División espacial en coocurrencias físicas, relaciones 2D y relaciones 3D para consolidar estimaciones de layout tridimensional.
...
* **Soluciones alcanzadas**: Uso de "tamaños familiares" relativos para descartar escalas físicamente imposibles de los objetos en función del tamaño del entorno.
...
* **Soluciones alcanzadas**: Clasificación en contextos de corto plazo y de largo plazo para propagar la consistencia de predicción en el tiempo.

### 3. Research method
Establece la metodología de la revisión sistemática, detallando bases de datos consultadas, rango de años y criterios de selección.
...
* **Limitaciones de ese entonces**: Revisiones previas eran narrativas e informales, omitiendo criterios de inclusión/exclusión claros o comparaciones rigurosas.
...
#### 4.1 Datasets
Revisión crítica de las bases de datos utilizadas para entrenar y evaluar modelos contextuales.
...
* **Limitaciones de ese entonces**: Dataset clásicos como COCO sesgan a los detectores hacia entornos urbanos occidentales, e ignoran dinámicas específicas.
...
Análisis de enfoques de integración contextual para detección genérica de imágenes estáticas.
...
* **Soluciones alcanzadas**: Clasificación estructurada y tabulación exhaustiva comparativa en COCO, VOC de docenas de modelos, demostrando qué variantes de grafos y jerárquicos dominan sus respectivas categorías.
...
* **Soluciones alcanzadas**: Análisis de 17 modelos específicos demostrando que la integración de pirámides conscientes del contexto y campos de sensación expandidos rescatan la señal de objetos diminutos.
...
* **Problemas atacados**: Degradación visual temporal de los objetos debido al desenfoque por movimiento y oclusiones instantáneas.
* **Limitaciones de ese entonces**: Los detectores de una etapa fallan al perder la caja en cuadros borrosos sucesivos.
* **Soluciones alcanzadas**: Modelos como Context Faster R-CNN y el uso de "contexto de movimiento" basado en flujo óptico para guiar el detector.
...
* **Problemas atacados**: Sobreajuste catastrófico y colapso de características de clases novedosas ante la escasez de anotaciones.
...
* **Soluciones alcanzadas**: Uso de contexto semántico global basado en embeddings textuales y transformadores de afinidad de contexto.
...
* **Problemas atacados**: Incapacidad de detectores tradicionales para delimitar objetos que no presentan contraste en sus contornos.
...
* **Soluciones alcanzadas**: Redes de fusión cruzada consciente del contexto, realce de bordes mediante transformadas de Fourier y estimación de importancia de píxeles.

### 5. Conclusion, research gaps, and limitations
Respuestas a las RQs y planteamiento de vacíos y limitaciones identificados.
* **Problemas atacados**: Falta de robustez en dominios cruzados, dependencia de contextos estáticos y validación estadística débil.
...
* **Soluciones alcanzadas**: Delineación de 9 vacíos de investigación crítica (modelos multimodales, conciencia de profundidad, cuantificación de incertidumbres).
