...
### 1. Introducción
Los Sistemas de Vigilancia de Tráfico (TSS) son esenciales para los Sistemas Inteligentes de Transporte (ITS). Las cámaras de vigilancia son el sensor predominante frente a espiras inductivas, radares y LiDARs debido a que proveen información semántica y visual de alta resolución a bajo costo. El avance de técnicas de aprendizaje profundo (CNNs y Transformers) ha superado la dependencia de descriptores manuales tradicionales (SIFT, HOG).
...
### 2. Overview
Describe la estructura general del artículo mapeando las secciones del ciclo de percepción (bajo nivel, alto nivel, limitaciones y tendencias de modelos fundacionales).
...
##### 3.1.1 2D Detection
Clasificación y comparación de detectores de dos etapas (Faster R-CNN, Cascade R-CNN) y de una etapa (YOLO, SSD, FCOS, CenterNet, RT-DETR).
...
* **Limitaciones de ese entonces**: Los detectores de dos etapas sufren de alta latencia inherente. Los detectores de una sola etapa basados en anclas (anchors) fallan ante orientaciones de objetos inusuales o densidades compactas.
...
* **Limitaciones de ese entonces**: Los métodos basados en restricciones geométricas puras (e.g., puntos de fuga) son sumamente sensibles a errores de calibración y movimientos físicos de la cámara.
* **Soluciones alcanzadas**: Adopción de métodos de estimación directa mediante aprendizaje profundo extremo a extremo (e.g., MonoUNI, BEVHeight) que estiman altura y profundidad normalizada para contrarrestar la incertidumbre espacial.
...
* **Limitaciones de ese entonces**: Los descriptores tradicionales (HOG, SIFT) colapsan ante ángulos oblicuos o variaciones de iluminación nocturna.
* **Soluciones alcanzadas**: Modelos basados en Transformers de visión (Swin-T) y cabezales híbridos multicapa que reconocen con más del 99% de precisión modelos y marcas.
...
* **Problemas atacados**: Discriminación ineficaz de vehículos con características globales idénticas (mismo modelo y color).
...
* **Soluciones alcanzadas**: Arquitecturas que combinan descriptores locales basados en partes distintivas con atenciones de grafos espaciales (e.g., GiT) e información auxiliar de orientación.
...
* **Problemas atacados**: Deriva del rastreador (drift) causada por cambios repentinos de iluminación y oclusiones temporales del objeto de interés.
...
* **Limitaciones de ese entonces**: Los enfoques desacoplados (SDT - SORT, DeepSORT) sufren de sobrecosto computacional secuencial y propagación de errores de detección. Los enfoques conjuntos (JDE - FairMOT) experimentan conflictos de aprendizaje de características entre la subtarea de detección y la de Re-ID.
* **Soluciones alcanzadas**: Desarrollo de modelos basados en transformadores espacio-temporales (TrackFormer, MeMOTR) que modelan explícitamente las interdependencias mutuas con atenciones de consulta (query attention).
...
* **Soluciones alcanzadas**: Compilación detallada de 16 datasets (como UA-DETRAC, VeRI-776, Stanford Cars, VisDrone) indicando tamaños, formatos de sensores y propósitos en el flujo TSS.
...
* **Soluciones alcanzadas**: Estandarización de métricas 3D específicas por benchmark (ATE, ASE en nuScenes; ACS, AGS en Rope3D) y métricas de seguimiento MOTA e IDF1.
...
* **Soluciones alcanzadas**: Algoritmos de auto-calibración online basados en puntos de fuga dinámicos de trayectorias (vanishing points) y aprendizaje automático de puntos clave vehiculares (vehicle keypoints).
...
* **Soluciones alcanzadas**: Uso de homografías dinámicas calculadas por redes que transforman trayectorias viales al plano del mundo real, reduciendo el margen de error a menos de 2 km/h.
...
* **Soluciones alcanzadas**: Clasificación de métodos en basados en seguimiento (dynamic tracking) y de regresión directa (ccNN, SRRNet), sugiriendo su selección complementaria según se requiera análisis microscópico o macroscópico.
...
Uso de anotaciones débiles (video-level labels) que indican la presencia de una anomalía en algún momento del video, sin precisar el instante ni la posición exacta.
...
* **Limitaciones de ese entonces**: Los modelos colapsan o se sobreajustan debido a que los datos de accidentes son escasos (imbalance crítico) y las anotaciones viales son ruidosas.
* **Soluciones alcanzadas**: Algoritmos basados en Aprendizaje de Instancias Múltiples (MIL) y redes de atención temporal (MIST, NTCN-ML) que localizan temporalmente la anomalía mediante clasificaciones dinámicas por bolsas.
...
* **Problemas atacados**: Incapacidad de predecir o detectar anomalías desconocidas o novedosas que no aparecen en conjuntos de entrenamiento supervisados.
* **Limitaciones de ese entonces**: Los codificadores automáticos simples (Autoencoders) sufren de "sobre-generalización", logrando reconstruir con bajo error fotogramas que contienen colisiones o movimientos inusuales.
* **Soluciones alcanzadas**: Autoencoders condicionados con módulos de memoria explícita de normalidad (MemAE) y predictores de fotogramas futuros basados en GANs/Transformers, donde un alto error de predicción temporal delata la anomalía.
...
* **Limitaciones de ese entonces**: Los modelos puramente cinemáticos (físicos) no modelan la interacción mutua entre vehículos y colapsan en plazos mayores a 2 segundos.
* **Soluciones alcanzadas**: Redes Transformer espacio-temporales aplicadas a la infraestructura (e.g., VT-Former) que integran atenciones cruzadas sociales para predecir trayectorias en horizontes mayores.

##### 4.3.2 Vulnerable Road User Behavior Understanding (VRBU)
Detección de la intención de cruce (CIR) y trayectoria futura de peatones y ciclistas.
...
* **Limitaciones de ese entonces**: Evaluar solo la trayectoria histórica no captura la intención real del usuario (e.g., mirar hacia el vehículo o postura corporal previa al paso).
* **Soluciones alcanzadas**: Enfoques híbridos que fusionan poses clave 3D estimadas en tiempo real con la trayectoria del peatón y contexto del entorno vial (e.g., PIP-Net, PedCMT).
...
* **Limitaciones de ese entonces**: La mayoría de los datasets de comportamiento peatonal se recopilan desde cámaras a bordo (dashcams) y no desde postes fijos de vigilancia.
* **Soluciones alcanzadas**: Recopilación de 17 conjuntos de datos clave (como CDD, CADP, BrnoCompSpeed, JAAD, PIE, V2X-Seq), documentando sus plataformas de sensores y relevancia vial.
...
Consolidación de las cinco barreras de los sistemas actuales: Degradación perceptiva (lluvia, noche, oclusiones), Restricciones de datos (falta de etiquetas masivas y eventos raros), Brecha semántica (falta de razonamiento causal en redes de caja negra), Cobertura de sensores (campo de visión limitado a cámaras individuales), y Demandas de cómputo (alto coste de inferencia de Transformers en el borde).
...
Revisión de soluciones de mitigación:
1. Mejora de la percepción (GANs, difusión y súper resolución de baja latencia).
2. Aprendizaje eficiente (pocos ejemplos, SSL y generación de datos sintéticos ultra-realistas).
3. Comprensión semántica (razonamiento neuro-simbólico y causalidad física).
4. Sensores cooperativos (colaboración multi-vista V2X e infraestructura V2I).
5. Cómputo eficiente (compresión, cuantización y optimización del cómputo en el borde).
* **Problemas atacados**: Alto coste computacional y desalineación entre la mejora visual y la precisión analítica posterior (gap percepción-rendimiento).
...
* **Soluciones alcanzadas**: Orientación de tendencias hacia optimizaciones conjuntas hardware-aware y alineación de pretext-tasks con la dinámica temporal de tráfico.

#### 5.3 Foundation Model Prospects
El impacto transformador de Large Foundation Models.
* **Problemas atacados**: Falta de adaptabilidad de los detectores supervisados ante categorías novedosas (como vehículos exóticos o desvíos temporales) y la brecha del razonamiento de seguridad.
...
* **Soluciones alcanzadas**: Uso de open-vocabulary detectors y modelos visuales-lenguaje (VLMs) para consultas visuales interactivas (VQA) y análisis de causa raíz en accidentes. Asimismo, modelos de mundo físico (FWMs como Sora) permiten simular dinámicas viales complejas para generar datos sintéticos realistas con correcta física de colisión.
