...
\begin{table*}[!t]
  ...
  \begin{tabular}{p{0.22\textwidth} p{0.18\textwidth} p{0.18\textwidth} p{0.34\textwidth}}
    ...
    \texttt{muestra\_300/} & 228.0 MB & 300 images & Local sample for visual inspection of frames and annotations. \\
    \bottomrule
  \end{tabular}
\end{table*}
...
The dataset is registered and distributed through the SMART Challenge 2026 competition page on Kaggle, hosted by Peru's Ministry of Transportation and Communications (MTC)~\cite{MTC2026SmartChallenge}. Access requires acceptance of the competition rules and Kaggle's terms of service. The raw data files---\texttt{train.zip}, \texttt{test.zip}, \texttt{train.csv}, and \texttt{sample\_submission.csv}---are not redistributed as part of this work. The code developed for data preprocessing, format conversion, and model training is released under the GPLv3 license, independently of the dataset distribution terms. The repository location and version identifier are provided in the Code Availability statement.
